  ...
  return (
    <div className="flex-1 relative w-full pt-8 px-8 overflow-y-auto">
      <header className="flex justify-between items-center mb-12">
        <div>
          <h1 className="text-4xl font-serif font-bold text-white mb-2">Good morning, Alex.</h1>
          <p className="text-white/60 text-lg">Here&apos;s what&apos;s happening with your notes.</p>
        </div>
        
        <div className="flex gap-4 items-center">
          <div className="relative group">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-white/40 group-focus-within:text-primary transition-colors" size={20} />
            <input 
              type="text" 
          ...
          >
            <Link href={`/notes/${note.id}`} className="block h-full">
              <div className="glass-card p-6 h-full flex flex-col cursor-pointer group">
                ...
                <p className="text-white/50 text-sm mb-6 flex-1 line-clamp-2">{note.excerpt}</p>

                <div className="flex justify-between items-center mt-auto pt-4 border-t border-surface-border/50">
                  <div className="flex gap-2">
                    {note.tags.map((tag) => (
                      <span key={tag} className="text-xs px-2 py-1 rounded bg-surface border border-surface-border text-white/70">
                        {tag}
                      </span>
                    ))}
                  </div>
